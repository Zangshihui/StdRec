...
@cython.boundscheck(False) 
@cython.wraparound(False)
def Grid(float BoxSize, int NMesh):
    Position = np.zeros([NMesh**3, 3], dtype = np.float32)
    cdef float H = BoxSize/NMesh
    ...
    with nogil, parallel():
        for I in prange(NMesh):
            for J in range(NMesh):
                for K in range(NMesh):
                    Position_view[I*NMesh**2+J*NMesh+K, 0] = H*I
                    Position_view[I*NMesh**2+J*NMesh+K, 1] = H*J
                    Position_view[I*NMesh**2+J*NMesh+K, 2] = H*K
